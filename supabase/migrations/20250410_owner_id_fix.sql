-- Fix the contractor_performance view to use owner_id instead of user_id
...
SELECT 
    ...
    c.owner_id, -- Changed from user_id to owner_id
    p.first_name,
    p.last_name,
    ...
    0 AS avg_time_spent_minutes, -- Default to 0 since we don't have the data
    0 AS avg_response_time_minutes, -- Default to 0 since we don't have the data
    COALESCE(rs.avg_rating, 0) AS avg_rating,
    COALESCE(rs.total_ratings, 0) AS total_ratings
FROM 
    public.contractors c
JOIN 
    public.profiles p ON c.owner_id = p.id -- Changed from user_id to owner_id
LEFT JOIN 
    job_stats js ON c.id = js.contractor_id
LEFT JOIN 
    rating_stats rs ON c.id = rs.contractor_id;

-- Fix the policies to use owner_id instead of user_id
...
USING (
    auth.uid() IN (
        SELECT owner_id FROM public.contractors c -- Changed from user_id to owner_id
        WHERE c.id = contractor_id
    )
);

-- Simplify the monthly performance function to avoid time tracking
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            ...
            ELSE 0
        END AS completion_rate,
        0::NUMERIC AS response_time -- Default to 0 since we don't have the data
    FROM 
        monthly_jobs mj
    LEFT JOIN 
