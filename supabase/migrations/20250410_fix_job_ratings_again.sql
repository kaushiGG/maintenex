-- Fix the contractor_performance view to handle null time_spent values
...
WITH job_stats AS (
    SELECT 
        ...
        -- Handle case when time_spent might be null by coalescing with 0
        AVG(COALESCE(j.time_spent, 0)) AS avg_time_spent,
        -- Calculate response time more safely (handle null start_time)
        AVG(
            CASE 
...
SELECT 
    ...
    c.user_id, -- Using user_id instead of auth_id
    p.first_name,
    p.last_name,
...
JOIN 
    public.profiles p ON c.user_id = p.id -- Using user_id instead of auth_id
LEFT JOIN 
    job_stats js ON c.id = js.contractor_id
LEFT JOIN 
    rating_stats rs ON c.id = rs.contractor_id;

-- Fix the monthly performance function to handle null time values
...
BEGIN
    ...
    WITH monthly_jobs AS (
        SELECT 
            ...
            COUNT(*) AS total_jobs,
            COUNT(CASE WHEN j.status = 'completed' THEN 1 END) AS completed_jobs,
            -- Calculate average response time more safely
            AVG(
                CASE 
                    WHEN j.start_time IS NOT NULL AND j.created_at IS NOT NULL 
                    THEN EXTRACT(EPOCH FROM (j.start_time - j.created_at))/60
