-- Minimal script to check and create essential tables
-- Each statement is independent to avoid nested DO blocks or complex syntax
...
-- Enable RLS for business_sites
ALTER TABLE public.business_sites ENABLE ROW LEVEL SECURITY;

-- Create policies for business_sites
DROP POLICY IF EXISTS "Sites are viewable by authenticated users" ON public.business_sites;
CREATE POLICY "Sites are viewable by authenticated users" ON public.business_sites
    FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Sites can be inserted by authenticated users" ON public.business_sites;
CREATE POLICY "Sites can be inserted by authenticated users" ON public.business_sites
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Sites can be updated by authenticated users" ON public.business_sites;
...
-- Enable RLS for contractors
ALTER TABLE public.contractors ENABLE ROW LEVEL SECURITY;

-- Create policies for contractors
DROP POLICY IF EXISTS "Contractors are viewable by authenticated users" ON public.contractors;
CREATE POLICY "Contractors are viewable by authenticated users" ON public.contractors
    FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Contractors can be inserted by authenticated users" ON public.contractors;
CREATE POLICY "Contractors can be inserted by authenticated users" ON public.contractors
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Contractors can be updated by authenticated users" ON public.contractors;
CREATE POLICY "Contractors can be updated by authenticated users" ON public.contractors
    FOR UPDATE USING (auth.role() = 'authenticated');

-- Create site_contractors table
...
-- Enable RLS for site_contractors
ALTER TABLE public.site_contractors ENABLE ROW LEVEL SECURITY;

-- Create policies for site_contractors
DROP POLICY IF EXISTS "Site contractors are viewable by authenticated users" ON public.site_contractors;
CREATE POLICY "Site contractors are viewable by authenticated users" ON public.site_contractors
    FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Site contractors can be inserted by authenticated users" ON public.site_contractors;
CREATE POLICY "Site contractors can be inserted by authenticated users" ON public.site_contractors
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Site contractors can be updated by authenticated users" ON public.site_contractors;
CREATE POLICY "Site contractors can be updated by authenticated users" ON public.site_contractors
    FOR UPDATE USING (auth.role() = 'authenticated');

-- Create site_floor_plans table
...
-- Enable RLS for site_floor_plans
ALTER TABLE public.site_floor_plans ENABLE ROW LEVEL SECURITY;

-- Create policies for site_floor_plans
...
DROP POLICY IF EXISTS "Floor plans can be inserted by authenticated users" ON public.site_floor_plans;
CREATE POLICY "Floor plans can be inserted by authenticated users" ON public.site_floor_plans
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Floor plans can be updated by authenticated users" ON public.site_floor_plans;
